...
import { Radio, MessageSquare, Settings, Home as HomeIcon, MonitorPlay } from "lucide-react";
      ...
      <aside className="w-72 bg-[#090D16] border-r border-zinc-800/80 p-6 flex flex-col justify-between hidden md:flex sticky top-0 h-screen">
        <div className="space-y-6">
          ...
          <nav className="space-y-1.5">
            ...
            <Link href="/transmissoes" className="flex items-center gap-3 px-3.5 py-3 rounded-xl text-sm font-semibold bg-blue-600 text-white shadow-lg shadow-blue-600/25 transition-all">
      ...
      <main className="flex-1 p-6 md:p-8 overflow-y-auto">
        <div className="max-w-6xl mx-auto space-y-6">
          <header className="flex justify-between items-center bg-[#090D16] border border-zinc-800/80 px-6 py-4 rounded-[2rem]">
            ...
            <Link 
              href="/"
              className="bg-zinc-800 hover:bg-zinc-700 text-zinc-200 font-bold px-5 py-2.5 rounded-xl text-sm transition-all flex items-center gap-2 cursor-pointer"
            >
          ...
          <div className="bg-[#090D16] border border-zinc-800/80 rounded-[2.5rem] p-6 shadow-2xl space-y-4">
            <div className="flex justify-between items-center px-2">
              <div className="flex items-center gap-2">
                <span className="w-3 h-3 rounded-full bg-red-500 animate-pulse"></span>
                <h2 className="text-base font-bold text-white tracking-wide">Assistindo: {activeChannel.name}</h2>
              </div>
            ...
            <div className="relative w-full aspect-video rounded-2xl overflow-hidden bg-black border border-zinc-800 shadow-inner flex items-center justify-center">
              <iframe 
                name="Player" 
              ...
              {channels.map((channel) => {
                ...
                return (
                  <button
                    ...
                    className={`p-4 rounded-2xl text-sm font-bold transition-all flex flex-col items-center justify-center gap-2 border cursor-pointer shadow-lg ${
                      isSelected 
                  ...
                  >
                    <MonitorPlay className={`w-5 h-5 ${isSelected ? "text-white" : "text-blue-500"}`} />
